...
import { checkRbacCommand } from "../auto-reply/reply/rbac-native-guard.js";
    ...
    if (typeof (bot as unknown as { command?: unknown }).command !== "function") {
      ...
      for (const command of nativeCommands) {
        bot.command(command.name, async (ctx: TelegramNativeCommandContext) => {
          ...
          // RBAC: block native Telegram commands for non-admin users.
          const _rbacNative = checkRbacCommand({
            cfg,
            senderId: String(senderId ?? ""),
            command: `/${command.name.toLowerCase()}`,
          });
          if (_rbacNative.blocked) {
            await withTelegramApiErrorLogging({
              operation: "sendMessage",
              runtime,
              fn: () =>
                bot.api.sendMessage(
                  chatId,
                  _rbacNative.response ?? "Command not available.",
                  threadParams,
                ),
            });
            return;
